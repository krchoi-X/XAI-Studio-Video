...
**결정 (2026-09-18): 3번으로 확정한다.** 재시도로 판단할 사안이 아니었다. 첫 실제 작업에서
토큰이 무효화되었고 이어서 프로젝트에 약관 위반 경고까지 발부되었다. 데이터센터에서의 대량
전송에는 오브젝트 스토리지가 맞는 도구이고, 소비자용 드라이브를 OAuth로 그 용도에 쓰는 것이
애초에 잘못된 선택이었다.

드라이브는 사용자가 원래 쓰던 대로 **백업과 장기 보관**에 둔다. 세션 중 전송 경로에는 올리지
않는다. 드라이브 인증 복구(`rclone config reconnect`)는 보관 용도로 여전히 유효하다.
