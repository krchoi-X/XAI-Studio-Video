...
### 실측 (2026-09-18, 1차 시도 — 중단됨)

RunPod CPU pod에서 실행. 단일 4GB 업로드 **47 MB/s (약 380 Mbps)** 를 기록한 뒤, 나머지 4개
구간이 전부 실패했다.

원인은 스크립트도 네트워크도 아니었다. **구글이 "평소와 다른 액세스"로 판단해 2차 인증을
요구하며 토큰 접근을 차단**했다. 데이터센터 IP에서 방금 승인된 미심사 앱이 4GB와 파일 수백
개를 연달아 올리는 것은 소비자 계정 이상 탐지에 정확히 걸리는 패턴이다.

### 경고 — 합성 데이터로 드라이브를 벤치마크하지 말 것

후속 결과: 위 벤치마크 실행 이후 해당 **Google Cloud 프로젝트에 서비스 약관 위반 경고**가
발부되었고 정지 가능성이 통보되었다.

원인은 벤치마크 설계 자체다. `/dev/urandom` 으로 만든 무의미한 4GB와 200개 파일을
데이터센터 IP에서, 방금 만든 미심사 테스트 모드 앱으로 연달아 업로드했다. 이 조합은 드라이브를
범용 전송/덤프 용도로 남용하는 패턴과 구별되지 않는다. 게다가 토큰이 중간에 무효화되어
스크립트의 정리 단계(`rclone purge`)가 실행되지 못했고, 랜덤 데이터가 드라이브에 그대로
남았다 — 이것이 저장소 남용 신호를 계속 유지시켰다.

**이 형태의 측정을 반복하지 않는다.** 소비자용 클라우드 스토리지는 합성 대용량 트래픽으로
성능을 재는 대상이 아니다. 필요하다면 실제 산출물로, 실제 사용 규모에서, 한 번만 관찰한다.

사고 발생 시 조치 순서: 드라이브에 남은 테스트 데이터를 즉시 삭제하고 휴지통을 비운 뒤,
통보된 양식에 사실관계를 기재해 제출한다. 정지 대상은 Cloud 프로젝트(API 접근)이며 구글 계정과
드라이브 파일 자체가 아니지만, 방치하면 확대될 수 있다.

### 이것은 버그가 아니라 구조적 제약이다

**개인용 Gmail 드라이브를 데이터센터에서 자동 접근하는 것은 무인 운용과 잘 맞지 않는다.**

- pod은 매 세션 다른 데이터센터 IP에서 뜨고, 리전이 다르면 국가도 달라진다;
- 테스트 모드의 미심사 앱은 더 보수적으로 취급된다;
- 차단을 푸는 데 **사람의 승인**이 필요하므로, 무인 운용이라는 전제 자체가 깨진다.

완화와 대안을 순서대로:

1. **보안 알림 승인 + 앱 게시.** 테스트 모드에서 벗어나는 것만으로 상당히 완화되고, 7일 토큰
   만료 문제도 함께 해결된다. 먼저 이것부터 한다.
2. **리전 고정.** pod을 항상 같은 RunPod 데이터센터에서 띄우면 IP 대역이 안정되어 이상 탐지가
   줄어든다.
3. **재발하면 세션 중 전송을 S3 호환 스토리지로 옮긴다.** 오브젝트 스토리지는 데이터센터
   접근을 전제로 설계되어 이런 휴리스틱이 없다. 드라이브는 장기 보관 전용으로 남긴다.

**확정된 것 (2026-09-18):** 드라이브를 세션 중 자동 전송 경로로 쓰지 않는다. 첫 실제 작업에서
토큰이 무효화되었고 프로젝트에 약관 위반 경고까지 이어졌다. 드라이브는 사용자가 원래 쓰던 대로
**백업과 장기 보관**에 둔다. 인증 복구(`rclone config reconnect`)는 그 용도로 여전히 유효하다.

**아직 결정되지 않은 것: 그 자리를 무엇이 대신하는가.** 새 외부 서비스와 반복 비용이 걸린
선택이므로 사용자가 정한다. 후보:

| 후보 | 저장 | 다운로드 | 비고 |
|---|---|---|---|
| Cloudflare R2 | ~$0.015/GB/월 | 무료 | 10GB 무료 한도. 키+시크릿만 필요 |
| Backblaze B2 | ~$0.006/GB/월 | 저장량 3배까지 무료 | 저장 단가가 더 낮음 |
| AWS S3 | ~$0.023/GB/월 | ~$0.09/GB | 반복 다운로드에 불리 |
| 도입하지 않음 | 0 | 0 | 산출물을 pod에서 선별해 소량만 드라이브로. 무인 운용은 포기 |

**규모를 줄이면 비용은 거의 사라진다.** 가중치는 HuggingFace에서 직접 받고(공개 모델이고 원래
그 용도다), 캐릭터 입력은 수십 MB 수준이므로, 오브젝트 스토리지가 담을 것은 세션 산출물
20~50GB 정도다. R2 기준 월 $1 이하다.
