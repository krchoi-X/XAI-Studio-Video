...
**대체 저장소는 도입하지 않는다 (2026-09-18 결정).** 애초에 대량 전송이 필요 없다는 것이
확인되었기 때문이다.

측정 결과 `characters/` 는 **18MB 전량 텍스트**(json 1841, txt 433, jsonl 378, md 132,
yaml 80, 이미지 0개)이고 저장소 전체가 21MB다. pod에서의 `git clone` 은 2초였다. 즉 캐릭터
입력은 스토리지 문제가 아니라 **이미 해결된 문제**였다.

산출물도 마찬가지다. 플로우는 "많이 생성 -> 갤러리에서 선별 -> 고른 것만 남김"이므로 durable
저장이 필요한 것은 선별된 소수뿐이다. 세션 말미의 수백 MB 실제 이미지 업로드는 이번에 차단된
패턴(데이터센터발 랜덤 데이터 4GB 연속 전송)과 전혀 다른 프로파일이다.

최종 구성:

| 대상 | 경로 |
|---|---|
| 캐릭터 시트 | `git clone` (18MB, 2초) |
| 레퍼런스 이미지 | 소량. git 또는 세션 시작 시 드라이브에서 |
| 모델 가중치 | HuggingFace 직접 (공개 모델, 본래 용도) |
| 산출물 | pod 갤러리에서 선별 후 선별본만 드라이브로 |

새 외부 계정은 추가하지 않는다.

### GitHub를 산출물 저장소로 쓰지 않는 이유

입력에는 이미 쓰고 있지만 산출물에는 맞지 않는다. 미디어를 일반 git에 넣으면 히스토리에
영구히 남아 되돌릴 수 없고, 이는 `repository-backup-policy.md` 의 "Never ordinary Git" 규칙에
정면으로 어긋난다. LFS는 저장과 대역폭이 모두 과금되어 영상에 부적합하고, Releases 와 GHCR 은
무료지만 공개이므로 캐릭터 미디어를 둘 수 없다.

### 감수하는 손실

pod이 예기치 않게 죽으면 그 세션의 **미선별 생성물**이 사라진다. 다만 그것들은 대부분 폐기
대상이고, 손실 비용은 재생성이지 복구 불가능한 자산이 아니다. 선별된 결과는 선별 시점에
올린다.
