...
Each item is prepared once through `character_scene.py`, which writes the exact DNA-merged `prompt.txt`, engine settings, Scene Spec, and provenance before GPU work starts. Rendering then reuses that session. If the detached worker loses the GPU-lock race, the queue observes the failed `run.json` and retries the same item after 10, 20, and 40 seconds. Other failures are not retried blindly. Failed run directories remain as evidence; a retry creates a new unique run. Before marking an item completed, the queue verifies the prompt and settings, terminal run state, artifact count, file existence, and that every artifact is under the declared engine output directory.

`basis: character-default` is not a general Krea2 completion requirement. It is the current automatic-reference marker for Ref2VA video. Ordinary Krea2 stills are driven by the compiled Character DNA prompt; reference-bound Krea2 editing uses explicit, hash-recorded `image_refs` through its separate transformation contract.

For an identity-sensitive still that the operator explicitly ties to a selected face, use the common scene command with `--engines krea2 --identity-reference character-default` or an explicit local image path. This switches only that job to `krea2_turbo_edit`, records the reference path/hash/size in the session and run, and refuses mixed engines or a missing reference instead of falling back to text-only Krea2. `--reference-asset-id` may accompany the reference when an approved adapter supplied a durable asset ID; never invent one. All actors use the same option and their real `--actor` value.
