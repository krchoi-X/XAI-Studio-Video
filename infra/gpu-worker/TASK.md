...
Owner: Claude
Active editor / integration owner: Codex
Status: PARTIAL — audited RunPod lifecycle integrated; remaining Phase 1 items are explicit
Started: 2026-09-18
...
- 2026-09-24 verification: `test_provision.py` passes 22 tests with the Studio backend
  virtualenv; `provision.py` compiles; `bench-drive.sh` passes Git Bash `bash -n`; staged
  diff check passes. Official RunPod v2 documentation was checked for pod list/get/delete,
  network-volume list, billing query, `cost`, and the 204 delete response. No live provider,
  GPU, billable resource, volume deletion, or credential was used.
- 2026-09-24 Codex integration review: merge against current `main` conflicts only in
  `provision.py`, where `main` independently carried the earlier `list-active` subset.
  Preserve Claude's full lifecycle commands and current main's fail-closed audit behavior.
  Fix two review findings before integration: record a successfully created RunPod pod in
  `active-resources.json`, and never treat a non-404 verification error as proof that a pod
  is gone. Official v2 review also found that hourly cost is returned as `cost`, not only the
  legacy aliases, and that pod termination is available at the v2 endpoint; cover both. Add
  deterministic regression tests. Make the local active-resource receipt atomic and fail visibly
  if it cannot be persisted; make no provider or billable call.

- 2026-09-18: Claude implemented the RunPod portion of Phase 1. Added `runpod-status`,
  `wait-ready`, `runpod-terminate` and
...
for Vast (`vast-status`, `vast-destroy`) and the WanGP model-schema readiness check still
follow. Do not call Phase 1 complete until those are implemented and deterministically tested.
